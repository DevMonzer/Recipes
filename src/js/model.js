...
export const loadSearchResults = async function (query) {
  try {
    ...
    // Resest the page number back to 1
    state.search.page = 1;
  } catch (err) {
    console.error(`${err} 💥💥💥💥`);
    throw err;
...
export const getSearchResultsPage = function (page = state.search.page) {
  ...
  const end = page * state.search.resultsPerPage; // 9 ( page time the number of results we want per page )
...
export const updateServings = function (newServings) {
  ...
  // Update the servings in the state so we track the new servings values
...
export const addBookmark = function (recipe) {
  ...
  // Save the bookmark to the localStorage
  persistBookmarks();
};
...
export const deleteBookmark = function (id) {
  ...
  // Delete the bookmark from the localStorage
  persistBookmarks();
};

// Get back the bookmarks from the localStorage and update the bookmarks state
const init = function () {
